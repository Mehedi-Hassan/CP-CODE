...
//    freopen("output.txt", "w", stdout);
    ...
    f(i, 1, n-1){
        ...
        if(pre[d] == 0){
            ...
            YES;
            // cout<<a[pre[d] - 1].second<<" "<<a[pre[d]].second<<" "<<a[i - 1].second<<" "<<a[i].second<<endl;
            cout<<a[pre[d] - 1].second<<" "<<a[i].second<<" "<<a[i - 1].second<<" "<<a[pre[d]].second<<endl;
            rr;
        }
    }

    map<int, vii>dif, left;
    ...
    for(auto mp:dif){
        if(mp.second.size() == 1) continue;
        int d = mp.first;

        int x = dif[d][0].first, y = dif[d][0].second, z = dif[d].back().first, w = dif[d].back().second;

        if(x!=z && x!=w && y!=z && y!=w){
            YES;
            cout<<a[x].second<<" "<<a[w].second<<" "<<a[z].second<<" "<<a[y].second<<endl;
            rr;
        }
    }
