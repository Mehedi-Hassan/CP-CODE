...
int get(int mask, int nextMask)
{
    if((mask==0 && nextMask!=0) || (mask==3 && nextMask!=3))
        return 1;
    if((mask==1 && nextMask==2) || (mask==2 && nextMask==1))
        return 2;
    return 0;
}
signed main()
    ...
    for(int i=1; i<=n;i++){
        for(int k=1; k<=2*n;k++){
            for(int mask=0; mask<=3; mask++){
                for(int nextMask=0; nextMask<=3; nextMask++){
                    dp[i+1][k + get(mask, nextMask)][nextMask] = modAdd(dp[i+1][k + get(mask, nextMask)][nextMask] , dp[i][k][mask]);
//                    dp[i+1][k + get(mask, nextMask)][nextMask] %= MOD;
                }
            }
        }
    }
