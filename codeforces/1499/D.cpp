...
void sieve () {
    siv[1] = 1; // 0 and 1 are not prime

    // prime.push_back(2); // Only Even Prime
    for ( int i = 2; i < svMX; i += 2 ) siv[i] = 1, fact[i] = 1; // Remove multiples of 2

    int sqrtn = sqrt ( svMX );
    for ( int i = 3; i <svMX; i += 2 ) {
        if ( siv[i] == 0 ) {
            for ( int j = i; j < svMX; j += i ) siv[j] = 1, fact[j]++;
        }
    }
}
