...
//    freopen("output.txt", "w", stdout);
    ...
    f(tc, 1, T){
        ...
        f(mask, 0, 15){
            int pu = u, pr = r, pd = d, pl = l;
            int au[n+1], ar[n+1], ad[n+1], al[n+1];
            mem(al);
            mem(ar);
            mem(au);
            mem(ad);

            if(mask & (1ll<<0)) au[n] = ar[1] = 1, u--, r--;
            if(mask & (1ll<<1)) ar[n] = ad[1] = 1, r--, d--;
            if(mask & (1ll<<2)) ad[n] = al[1] = 1, d--, l--;
            if(mask & (1ll<<3)) al[n] = au[1] = 1, l--, u--;
