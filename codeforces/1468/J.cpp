...
int dsfind(int r)
{
    if (dspar[r] == r) return r;
    return dspar[r] = dsfind(dspar[r]);
}
...
//    T = 1;
    ...
    for(int tc = 1;tc<=T; tc++){
        ...
        vector<pair<int, pii>>edge1, edge2;
        f(i, 1, m){
            // cin>>x>>y;
            sf(x), sf(y), sf(z);
            if(z<=k)
        ...
        int cnt = 0, mn = inf, sum = inf;
        for(auto x:edge1){
            if(dsunion(x.se.fi, x.se.se)){
                cnt++;
            }
            mn = min(mn, k-x.fi);
        }
