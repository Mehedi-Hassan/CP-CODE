...
//    T = 1;
    ...
    for(int tc = 1;tc<=T; tc++){
        ...
        int mn = h[1]+k, mx = h[1]+k, bad = 0;

        f(i, 2, n-1){
            if((mn-(k-1) > h[i]+2*k-1) || (mx+k-1 < h[i]+k)){
                bad = i;
                break;
            }
            mn = min(max(mn-(k-1), h[i]+k), h[i]+2*k -1);
            mx = max(min(mx+(k-1), h[i]+2*k -1), h[i]+k);
        }
        if((mn-(k-1) > h[n]+k) || (mx+k-1 < h[n]+k)){
            bad = n;
        }
