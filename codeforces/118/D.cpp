...
int getans(int f, int h, int rem1, int rem2)
{
    if(f+h == 0){
        return dp[f][h][rem1][rem2] = 1;
    }

    if(dp[f][h][rem1][rem2] != -1)
        return dp[f][h][rem1][rem2];
    int x=0, y=0;

    if(f>0 && rem1>0)
        x = getans(f-1, h, rem1-1, k2);
    if(h>0 && rem2>0)
        y = getans(f, h-1, k1, rem2-1);

    return dp[f][h][rem1][rem2] = modAdd(x, y);
}
