...
//    freopen("output.txt", "w", stdout);
    ...
    f(tc, 1, T){
        ...
        int a[n+1], b[n+1];
        stack<int>done[n+1], lage[n+1], extraid;
        // mem(done);
        // mem(lage);
        ...
        f(i, 1, m){
            ...
            if(!lage[x].empty()){
                ans.pb(lage[x].top());
                while(!extraid.empty()){
                    ans[extraid.top()] = lage[x].top();
                    // extra.pop();
                    extraid.pop();
                }
                done[x].push(lage[x].top());
                lage[x].pop();
            }
            else if(!done[x].empty()){
                ans.pb(done[x].top());
                while(!extraid.empty()){
                    ans[extraid.top()] = done[x].top();
                    // extra.pop();
                    extraid.pop();
                }
                // done[x].pop();
