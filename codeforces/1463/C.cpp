...
//    T = 1;
    ...
    for(int tc = 1;tc<=T; tc++){
        ...
        int busytill, age=0, pore, dir = 1, des = x[1];

        busytill = t[1] + abs(x[1] - age);
        if(x[1]<0)
            dir = -1;
        ...
        f(i, 2, n){
            if(dir<0){
                pore = max(age + dir*(t[i] - t[i-1]), des);
                if(pore<=x[i-1] && x[i-1]<=age)
                    ans++;
                age = pore;
            }
            else{
                pore = min(age + dir*(t[i] - t[i-1]), des);
                if(age<=x[i-1] && x[i-1]<=pore)
                    ans++;
                age = pore;
            }
