...
//    T = 1;
    ...
    for(int tc = 1;tc<=T; tc++){
        ...
        int ans = 0;
        f(i, 1, n){
            int andsum = 0, orsum = 0, d = 1, x;
            x = a[i];
            f(id, 0, 61){
                if(x%2){
                    andsum = modAdd(andsum, modMul(pos[id], d));
                    orsum = modAdd(orsum, modMul(n, d));
                }
                else
                    orsum = modAdd(orsum, modMul(pos[id], d));
                d *= 2;
                x /= 2;
            }

            ans = modAdd(ans, modMul(orsum, andsum));
//            error(orsum)
        }
