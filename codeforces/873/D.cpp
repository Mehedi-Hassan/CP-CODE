...
//    root = new trinode();
    ...
    while(k>0 && !q.empty() && lim>0){
        ...
        if(x.se-x.fi > 1){
            ...
            par[pii(x.fi, mid)] = par[pii(mid, x.se)] = x;
            k-=2;
        }
        else{
            q.push(x);
    ...
    for(int i=0;i<n;i++){
        if(i+1 < n){
            if(par[ans[i]] == par[ans[i+1]]){
                for(int j=ans[i].fi+1; j<=ans[i].se-1;j++)
                    cout<<j<<" ";
                cout<<ans[i+1].fi+1<<" ";
                cout<<ans[i].se<<" ";
                for(int j=ans[i+1].fi+2; j<=ans[i+1].se;j++)
                    cout<<j<<" ";
                i++;
            }
            else{
                for(int j=ans[i].fi+1; j<=ans[i].se;j++)
                    cout<<j<<" ";
            }
        }
        else{
